...
Book *Library::lookupBook(const string &author, const string &title) {
    for(Book *bookPtr : m_bookPtrList){
        if(bookPtr->author()== author && bookPtr->title() == title) {
            return bookPtr;
        }
    }
...
Customer *Library::lookupCustomer(const string &name, const string &address) {
    for(Customer *customerPtr : m_customerPtrList) {
        if(customerPtr->name() == name && customerPtr->address() == address) {
            return customerPtr;
        }
    }
...
void Library::deleteBook() {
    ...
    list<Customer*> reservationPtrList = bookPtr->reservationPtrList();
...
void Library::deleteCustomer() {
    ...
    if(customerPtr == nullptr) {
        cout << endl << "Customer " << name << " does not exists." << endl;
        return;
    }

    if(!customerPtr->loanPtrSet().empty()) {
        cout << "The customer " << customerPtr->name() << " has borrowed books and cannot be deleted." << endl;
        return;
    }
...
void Library::borrowBook() {
    ...
    Book *bookPtr = lookupBook(author, title);
    if(bookPtr == nullptr) {
        cout << endl << "There is no book " << title << " by " << author << "." << endl;
        return;
    }
...
void Library::reserveBook() {
    ...
    if(bookPtr->borrowerPtr() == nullptr) {
        cout << endl << "The book " << title << " by " << author << " has not been borrowed. "
...
void Library::returnBook() {
    ...
    if(!reservationPtrList.empty()) {
        ...
        bookPtr->borrowerPtr() = newCustomerPtr;
        newCustomerPtr->borrowBook(bookPtr);
        cout << endl << "Borrowed by " << newCustomerPtr->name() << endl;
...
Book *Library::lookupBookPtr(int bookIndex) {
    assert(bookIndex>=0 && bookIndex<((int)m_bookPtrList.size()));

    auto iterator = m_bookPtrList.begin();
    for(int cout=0; cout <bookIndex; ++cout) {
        ++iterator;
    }
...
Customer *Library::lookupCustomerPtr(int customerIndex) {
    assert(customerIndex>=0 && customerIndex<((int)m_customerPtrList.size()));

    auto iterator = m_customerPtrList.begin();
    for(int cout =0; cout<customerIndex; ++cout) {
        ++iterator;
    }
    return *iterator;
}
    ...
    if(outStream) {
        {
            ...
            outStream.write((char*)&bookPtrListSize, sizeof bookPtrListSize);
...

        {
            ...
            outStream.write((char*)&customerPtrListSize, sizeof customerPtrListSize);
        ...
        for(const Book *bookPtr : m_bookPtrList) {
            ...
            {
                const list<Customer*> &reservationPtrList = bookPtr->reservationPtrList();
                int reserveSetSize = reservationPtrList.size();
                outStream.write((char*)&reserveSetSize, sizeof reserveSetSize);
                for(const Customer *customerPtr : reservationPtrList) {
                    int customerIndex = lookupCustomerIndex(customerPtr);
                    outStream.write((char*)&customerIndex, sizeof customerIndex);
                }
            }
        }
        ...
        for(const Customer *customerPtr : m_customerPtrList) {
            {
                ...
                outStream.write((char*)&loanPtrSetSize, sizeof loanPtrSetSize);

                for(const Book *bookPtr : loanPtrSet) {
                    int bookIndex = lookupBookIndex(bookPtr);
                    outStream.write((char*)&bookIndex, sizeof bookIndex);
                }
            }

            {
                const set<Book*> &reservationPtrSet = customerPtr->reservationPtrSet();
                int reservationPtrSetSize = reservationPtrSet.size();
                outStream.write((char*)&reservationPtrSetSize, sizeof reservationPtrSetSize);

                for(const Book *reservationPtr : reservationPtrSet) {
                    int bookIndex = lookupBookIndex(reservationPtr);
                    outStream.write((char*)&bookIndex, sizeof bookIndex);
                }
            }
        }
...
void Library::load() {
    ...
    if(inStream) {
        {
            int bookPtrListSize;
            inStream.read((char*)&bookPtrListSize, sizeof bookPtrListSize);
            for(int cout=0; cout<bookPtrListSize; ++cout) {
...

        {
            ...
            inStream.read((char*)&customerPtrListSize, sizeof customerPtrListSize);
        ...
        for(Book *bookPtr : m_bookPtrList) {
...

            {
                ...
                for(int count=0; count<reservationPtrListSize; ++count) {
                    int customerIndex;
                    inStream.read((char*)&customerIndex, sizeof customerIndex);
                    Customer * customerPtr = lookupCustomerPtr(customerIndex);
        ...
        for(Customer *customerPtr : m_customerPtrList) {
            {
                ...
                int loanPtrSetSize = loanPtrSet.size();
                inStream.read((char*)&loanPtrSetSize, sizeof loanPtrSetSize);
            ...
            {
                set<Book*> &reservationPtrSet = customerPtr->reservationPtrSet();
                int reservationPtrSetSize = reservationPtrSet.size();
                inStream.read((char*)&reservationPtrSetSize, sizeof reservationPtrSetSize);
                for(int count=0; count<reservationPtrSetSize; ++count) {
                    int bookIndex;
                    inStream.read((char*)&bookIndex, sizeof bookIndex);
                    Book *bookPtr = lookupBookPtr(bookIndex);
